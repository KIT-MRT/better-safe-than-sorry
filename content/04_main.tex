...
\Cref{algo:verifying_arbitrator} shows the generic arbitration algorithm from \cite{lauer}
...
% https://github.com/KIT-MRT/safe_arbitrators_paper/pull/7/files#r1717325535
\todo{If there is space: Add a figure showing a seamlessly degrading arbitration graph.}
...
This command is therefore rejected by the verifier as well, and the system falls back to the last resort behavior \behavior{StayInPlace}.
...
Of course it would be possible to find the bug in \behavior{EatClosestDot} through testing prior to deployment in this toy example.
