...
\begin{algorithm}
  \SetKwFunction{BestOption}{BesteAnwendbareAktion}%
  ...
  \Fn{\BestOption{Situation $\situation$}}{\label{algo:verifying_arbitrator:bestOption}
    ...
    Sortiere anwendbare Optionen %mit der zugrundeliegenden Strategie:
  ...
  \;
  \While{true}{
    Bestimme die aktuelle Situation $\situation$\;\label{algo:verifying_arbitrator:situation}
    Bestimme $\verifiedCommand = (\command, \verification) = $ \BestOption{$\situation$}\;
    \If{Verifikation bestanden $\verification_a = 0$}{
      Führe $\command$ aus\;
    % }{
